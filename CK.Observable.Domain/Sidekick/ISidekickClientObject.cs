...
namespace CK.Observable
{
    ...
    /// Object that supports this interface are submitted to the given sidekick (see <see cref="ObservableDomainSidekick.RegisterClientObject(Core.IActivityMonitor, IDisposableObject)"/>)
    /// so that the sidekick and the object can collaborate.
    /// Note that the <see cref="UseSidekickAttribute"/> just enures that a sidekick must be up and running.
    /// </summary>
